...
这样写虽然能运行，但不推荐作为正式写法，原因是：

- 对象需要哪些属性不清晰
- 不同对象可能随手加出不同字段，后期难维护
- 不利于统一初始化和校验数据

更推荐在 `__init__` 中把对象需要的属性一次性定义好。
...
在实际开发中，`__str__` 很适合用来定义“对象打印出来长什么样”。
...
- `brand` 是实例属性，每个对象自己有一份
- `wheel` 是类属性，所有对象共享
- 通过实例访问属性时，会先找实例属性，再找类属性

#### 类方法

类方法使用 `@classmethod` 修饰，第一个参数通常写成 `cls`，表示当前类。
...
静态方法使用 `@staticmethod` 修饰，不依赖对象本身，也不依赖类本身，更适合放一些和类相关但不需要访问实例状态的工具逻辑。
...
- `self` 表示当前对象
- `__init__` 常用于初始化属性
- 继承用于复用，封装用于隐藏细节，多态用于统一接口
- 类属性、类方法、静态方法分别适合不同场景

## 第8章 强化练习

### 8.1 学生管理系统

这一节用面向对象的思想完成一个简化版教务管理系统。
...
为了符合面向对象思想，我们把系统拆成两个类：

1. `Student`
2. `EduManagement`

`Student` 负责表示一个学生对象。  
`EduManagement` 负责管理所有学生。
...
class Student:
    ...
    def __str__(self):
        return (
            f"姓名：{self.name} | 语文：{self.chinese} | 数学：{self.math} | "
...
- `system_version` 和 `system_name` 是类属性
- `student_list` 是实例属性，用来保存所有学生对象
...
这个案例非常适合用来巩固下面这些面向对象知识点：

- 如何定义类和对象
- 如何在 `__init__` 中初始化属性
- 如何用 `__str__` 定义对象输出格式
- 如何把“学生数据”和“系统管理逻辑”拆到不同类中
- 如何通过实例方法操作对象状态
- 如何在真实场景里使用列表保存多个对象
